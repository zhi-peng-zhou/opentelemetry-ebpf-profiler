...
var dir = filepath.Join(os.TempDir(), "otlp-hotspot-mem-profile")
var libPath = filepath.Join(dir, "hotspot_profiler.so")
...
func ExtractEmbeddedLibrary() error {
	...
	if err := os.WriteFile(libPath, embeddedLibLinux, 0755); err != nil {
...
func StartMemAllocProfilingOTLP(ctx context.Context, config *OTLPProfilerConfig, cha chan map[uint32]pprofile.Profiles) error {
	if err := ExtractEmbeddedLibrary(); err != nil {
		return fmt.Errorf("unable to extract embedded libraries: %w", err)
	}
	// 设置默认值
