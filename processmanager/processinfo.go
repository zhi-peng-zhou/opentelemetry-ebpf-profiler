...
func (pm *ProcessManager) processNewExecMapping(pr process.Process, mapping *process.Mapping) {
	...
	// todo  libc.so libc.musl libjemalloc.so libtcmalloc libmimalloc.so libdlmalloc.so libefence.so
	if strings.Contains(mapping.Path, "libc.so") {
		...
		if _info, ok := pm.pidToProcessInfo[pr.PID()]; ok {
			if _info.memProfileMeta != nil {
				...
				_info.memProfileMeta.LibcPath = libcPath
				_info.memProfileMeta.LibcPath = _info.meta.Executable
			}
		}
	}
